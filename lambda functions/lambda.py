
#================= serializeImageData Lambda ================#
import json
import boto3
import base64
import os

# s3 = boto3.client('s3')

def lambda_handler(event, context):
    """
        A function to serialize target data from S3

        Input structure:
        {
            "s3_key": "", # Source data S3 key
            "s3_bucket": "", # Source data S3 bucket
            "prefix":"", # Source data S3 prefix
        }

        eg of returned object:
        {
            "statusCode": 200,
            "body": {
                "image_data": "",
                "s3_bucket": "sagemaker-us-west-2-659095202386",
                "s3_key": "train/bicycle_s_000017.png",
                "inferences": [],
                "prefix": "udacity-project-4"
            }
        }
        
    """
    
    # Get the s3 address from the Step Function event input
    key = event.get("s3_key") # Source data S3 key (eg. "test/bicycle_s_000513.png")
    bucket = event.get("s3_bucket") # Source data S3 bucket
    prefix = event.get("prefix","udacity-project-4")  # Source data S3 key prefix
    
    # Download the data from s3 to /tmp/image.png
    download_folder="/tmp"
    file_name="image.png"
    download_from_s3(region="us-west-2",bucket=bucket,prefix=prefix,key=key,
                     download_folder=download_folder, file_name=file_name)
    
    # We read the data from a file
    with open(os.path.join(download_folder,file_name), "rb") as f: # with open("/tmp/image.png", "rb") as f:
        image_data = base64.b64encode(f.read())

    # Pass the data back to the Step Function
    print("Event:", event.keys())
    return {
        'statusCode': 200,
        'body': {
            "image_data": image_data,
            "s3_bucket": bucket,
            "s3_key": key,
            "inferences": [],

            "prefix":prefix
        }
    }


def download_from_s3(region:str,bucket:str,prefix:str,key:str,
                     download_folder:str,file_name:str)->None:
    # TODO: use the parameter region to download from custom region
    
    if not os.path.exists(download_folder):
        os.makedirs(download_folder)

    local_image = os.path.join(download_folder,file_name)
        
    s3_resource = boto3.resource("s3")
    s3_resource.Bucket(bucket).download_file(os.path.join(prefix, key), local_image)

    
#================= End of serializeImageData Lambda ================#

################################################################


#================= classifyImage Lambda ================#
import subprocess
import sys

# https://stackoverflow.com/questions/60311148/pip-install-python-package-within-aws-lambda
# pip install custom package to /tmp/ and add to path
subprocess.call('pip install sagemaker -t /tmp/ --no-cache-dir'.split(), stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
sys.path.insert(1, '/tmp/')

import json
import sagemaker
import base64
from sagemaker.serializers import IdentitySerializer

# Fill this in with the name of your deployed model
ENDPOINT = "image-classification-2023-09-07-12-14-33-954" 

def lambda_handler(event, context):
    """
        Input eg:
        
        "image_data": "",
        "s3_bucket": "sagemaker-us-west-2-659095202386",
        "s3_key": "train/bicycle_s_000017.png",
        "inferences": [],
        "prefix": "udacity-project-4"

        Output eg:
        {
            "statusCode": 200,
            "body": {
                "image_data": "",
                "s3_bucket": "sagemaker-us-west-2-659095202386",
                "s3_key": "test/ordinary_bicycle_s_000431.png",
                "inferences": [
                    0.46814629435539246,
                    0.5318536758422852
                ],
                "prefix": "udacity-project-4"
            }
        }



    """

    b64encoded_image=event.get("image_data")

    # Decode the image data
    image = base64.b64decode(b64encoded_image)

    # Instantiate a Predictor
    predictor = sagemaker.predictor.Predictor(ENDPOINT)

    # For this model the IdentitySerializer needs to be "image/png"
    predictor.serializer = IdentitySerializer("image/png")
    
    # predictor.serializer = IdentitySerializer("image/png")
    # with open("./test/bicycle_s_001789.png", "rb") as f:
    #     payload = f.read()
    # inference = predictor.predict(payload)

    # Make a prediction:
    inferences = predictor.predict(image) #eg. b'[0.91, 0.09]'
    inferences = inferences.decode("utf-8") #eg. '[0.91, 0.09]'
    
    # We return the data back to the Step Function    
    event["inferences"] = eval(inferences) #eg. [0.91, 0.09]
    return {
        'statusCode': 200,
        'body': event #json.dumps(event)
    }
#================= End of classifyImage Lambda ================#

################################################################

#================= inferencesFiltering Lambda ================#
import json


THRESHOLD = .8 #.93


def lambda_handler(event, context):

    """
        Filter low-confidence inferences. 
        Define a threshold between 1.00 and 0.000 for your model. If confidence is lower than threshold,
        an error is raised; otherwise we pass that inference along to downstream systems

           
        Input eg:
        
        "image_data": "",
        "s3_bucket": "sagemaker-us-west-2-659095202386",
        "s3_key": "train/bicycle_s_000017.png",
        "inferences": [0.46814629435539246, 0.5318536758422852],
        "prefix": "udacity-project-4"


        Output eg.

        {
            "statusCode": 200,
            "body": "{\"image_data\": \"\", \"s3_bucket\": \"sagemaker-us-west-2-659095202386\", \"s3_key\": \"train/bicycle_s_000017.png\", \"prefix\": \"udacity-project-4\", \"inferences\": [0.46814629435539246, 0.5318536758422852]}"
        }
  
    """
    
    # Grab the inferences from the event
    inferences = event.get("inferences",[0,0]) # json.loads(event.get("inferences","[]")) 
    
    # Check if any values in our inferences are above THRESHOLD
    meets_threshold = inferences[0]>= THRESHOLD or inferences[1]>= THRESHOLD
    
    # If our threshold is met, pass our data back out of the
    # Step Function, else, end the Step Function with an error
    if meets_threshold:
        pass
    else:
        raise("THRESHOLD_CONFIDENCE_NOT_MET")

    return {
        'statusCode': 200,
        'body': json.dumps(event)
    }
#================= End of inferencesFiltering Lambda ================#



